...
const Navbar = () => {
  ...
  return (
    <div className='fixed z-10 bg-violet-400 sm:h-[80px] top-0 left-0 right-0 py-3 px-[5%] flex flex-col sm:flex-row sm:justify-between items-center'>
      ...
      <div className='relative flex justify-center sm:justify-end'>
        <button className='relative' onClick={() => { setShowCart(!showCart) }}>
          {
            cart.length > 0 &&
            <span className='absolute top-0 right-0 bg-red-600 block h-4 text-xs text-white aspect-square rounded-full'>
              {cart.length}
            </span>
          }
